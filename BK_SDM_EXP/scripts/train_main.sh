#!/bin/bash
#SBATCH --job-name=BK_base_core
#SBATCH --partition=A6000 # A6000 , A100 , R4090 , R3090
#SBATCH --qos=gpu_qos_4 # a100_qos_4   , gpu_qos_8, a6000_qos_8
#SBATCH --gres=gpu:4
#SBATCH --cpus-per-task=32
#SBATCH --mem=32G
#SBATCH -o /workspace/hyuns/src/BK-SDM/slurm/o/%x.out
#SBATCH -e /workspace/hyuns/src/BK-SDM/slurm/e/%x.err
...
if [[ $# -lt 3 ]]; then
  echo "Usage: $0 <UNET_NAME> <LOSS_TYPE> <GROUP>"
...
OUTPUT_DIR="./results/${UNET_NAME}_${LOSS_TYPE}_${GROUP}"


# Batch/accumulation
# BATCH_SIZE=16                        # per-GPU
BATCH_SIZE="$BATCH_SIZE_PER_GPU"
TOTAL_BATCH_SIZE=256                 # BATCH_SIZE * GRAD_ACCUMULATION * NUM_GPUS
...
echo "===== Launch Config ====="
echo "UNET_NAME=$UNET_NAME  LOSS_TYPE=$LOSS_TYPE  GROUP=$GROUP"
...
accelerate launch \
  ...
  --loss_type "$LOSS_TYPE" --group_size "$PATCH" \
  "${EXTRA_ARGS[@]}"
