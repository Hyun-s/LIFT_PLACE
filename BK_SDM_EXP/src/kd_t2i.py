...
from funcs import MultiConv1x1, get_layer_output_channels, count_parameters, load_or_build_imagefolder_dataset
...
def LIFT_PLACE(
    et_s: torch.Tensor,
    et_t: torch.Tensor,
    group_size: int = 256, ## group_size=16 -> 16*16=256
    eps: float = 1e-4,
    use_l2: bool = True,
    ...
    assert total_pixels % group_size == 0, \
        f"Total pixels ({total_pixels}) must be divisible by group_size ({group_size})"
    num_groups = total_pixels // group_size
    ...
    s_grouped = s_sorted.view(B, C, num_groups, group_size)
    t_grouped = t_sorted.view(B, C, num_groups, group_size)
    ...
    recon_loss = ((eps_hat - t_grouped) ** 2) # Shape: [B, C, num_groups, group_size]
    ...
    if return_beta:
        return (loss_s + loss_i) + (1 - min(1, sch)) * recon_loss, (loss_s+ loss_i), slope, intercept
    else:
...
def copy_weight_from_teacher(unet_stu, unet_tea, student_type):
    ...
    elif student_type in ["bk_extreme"]:
        # connect_info['up_blocks.0.resnets.0.'] = 'up_blocks.1.resnets.0.'
        # connect_info['up_blocks.0.attentions.0.'] = 'up_blocks.1.attentions.0.'
        # connect_info['up_blocks.0.resnets.1.'] = 'up_blocks.1.resnets.2.'
        # connect_info['up_blocks.0.attentions.1.'] = 'up_blocks.1.attentions.2.'
        # connect_info['up_blocks.0.upsamplers.'] = 'up_blocks.1.upsamplers.'
        connect_info['up_blocks.0.resnets.0.'] = 'up_blocks.2.resnets.0.'
        connect_info['up_blocks.0.attentions.0.'] = 'up_blocks.2.attentions.0.'
        connect_info['up_blocks.0.resnets.1.'] = 'up_blocks.2.resnets.2.'
        connect_info['up_blocks.0.attentions.1.'] = 'up_blocks.2.attentions.2.'
        connect_info['up_blocks.0.upsamplers.'] = 'up_blocks.2.upsamplers.'
        connect_info['up_blocks.1.resnets.0.'] = 'up_blocks.3.resnets.0.'
        connect_info['up_blocks.1.attentions.0.'] = 'up_blocks.3.attentions.0.'
        connect_info['up_blocks.1.resnets.1.'] = 'up_blocks.3.resnets.2.'
        connect_info['up_blocks.1.attentions.1.'] = 'up_blocks.3.attentions.2.'     
    else:
    ...
    for k in unet_stu.state_dict().keys():
        ...
        for prefix_key in connect_info.keys():
            print(prefix_key, connect_info[prefix_key])
...
def parse_args():
    ...
    parser.add_argument("--unet_config_name", type=str, default="bk_small", choices=["bk_base", "bk_small", "bk_tiny", "bk_extreme","C_base", "C_small", "C_tiny", "C_micro","C_extreme"])   
...
def main():
    ...
    if args.unet_config_name in ["C_base", "C_small", "C_tiny", "C_micro","C_extreme"]:
    ...
    elif args.unet_config_name in ["bk_extreme"]:
        mapping_layers_tea = ['down_blocks.0', 'down_blocks.1',
                             'up_blocks.2', 'up_blocks.3']    
        mapping_layers_stu = ['down_blocks.0', 'down_blocks.1',
    ...
    # dataset = load_or_build_imagefolder_dataset(data_dir=args.train_data_dir, split="train")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.loss_type == "LIFT_PLACE":           
                    loss_kd_output, lift, slope, intercept = LIFT_PLACE(et_s = model_pred.float(), 
                                                                 et_t = model_pred_teacher.float(), 
                                                                 group_size = args.group_size, 
                                                                 eps = 1e-8, 
                                                                 use_l2 = False,return_beta=True)
                    
                    outkd = F.mse_loss(model_pred.float(), model_pred_teacher.float(), reduction="mean")
                else: # OutKD
                    with torch.no_grad():
                        loss_kd_output, lift, slope, intercept = LIFT_PLACE(et_s = model_pred.float(), 
                                                                    et_t = model_pred_teacher.float(), 
                ...
                ###################
                avg_lift = accelerator.gather(lift.repeat(args.train_batch_size)).mean()
                # avg_lift = accelerator.gather(lift).mean()
                train_lift += avg_lift.item() / args.gradient_accumulation_steps

                avg_outkd = accelerator.gather(outkd.repeat(args.train_batch_size))
                train_outkd += avg_outkd.item() / args.gradient_accumulation_steps

                avg_slope = accelerator.gather(slope).mean()
                train_slope += avg_slope.item() / args.gradient_accumulation_steps

                avg_intercept = accelerator.gather(intercept).mean()
                train_intercept += avg_intercept.item() / args.gradient_accumulation_steps
                ###################
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    with open(csv_log_path, 'a') as logfile:
                        logwriter = csv.writer(logfile, delimiter=',')
                        logwriter.writerow([epoch, step, global_step,
                                            train_loss, train_loss_sd, train_loss_kd_output, train_lift, train_outkd, train_loss_kd_feat,
                                            train_slope, train_intercept,
            ...
            logs = {"step_loss": loss.detach().item(),
                    ...
                    "lift_loss": lift.detach().item(),
                    "slope": slope.detach().mean().item(),
                    "intercept": intercept.detach().mean().item(),
                    "outkd": outkd.detach().item(),
