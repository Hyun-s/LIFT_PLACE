...
class Diffusion(object):
    ...
    def train(self, kd=False):
        ...
        optimizer = SAM(model.parameters(), base_optimizer, lr=config.optim.lr, weight_decay=config.optim.weight_decay,
        ...
        if args.kd:
            if args.loss_type =='fitnet':
                ...
                kd_loss.loss = loss
            elif args.loss_type == 'itgc':
                model, loss, optimizer,ebm_opt, train_loader = accelerator.prepare(
                    model, kd_loss.loss, 
                    optimizer, kd_loss.loss.ebm_opt , 
                    train_loader
                )
                kd_loss.loss = loss
                kd_loss.loss.ebm_opt = ebm_opt
            else:
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                if kd:
                    ...
                    # base setting is 0.3*simple + 0.7*logit
    ...
    def sample_image(self, x, model, last=True):
        ...
        # elif self.args.sample_type == "corrected_ddim":
        #     if self.args.skip_type == "uniform":
        #         skip = self.num_timesteps // self.args.timesteps
        #         seq = range(0, self.num_timesteps, skip)
        #     elif self.args.skip_type == "quad":
        ...
        #     else:
        #         raise NotImplementedError
        #     from functions.denoising import corrected_generalized_steps

        #     xs = corrected_generalized_steps(x, seq, model, self.teacher, self.betas, eta=self.args.eta)
        #     x = xs
