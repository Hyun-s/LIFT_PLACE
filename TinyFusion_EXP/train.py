    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            ...
            if torch.isnan(x).any() or torch.isinf(x).any():
                raise RuntimeError("x에 NaN/Inf가 섞여 있습니다!")
            opt.zero_grad()
            t = torch.randint(0, diffusion.num_timesteps, (x.shape[0],), device=device)
            model_kwargs = dict(y=y)
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                loss_dict = diffusion.training_losses(model, x, t, model_kwargs, args=args)
            ...
            # running_reg_loss += loss_dict["reg_loss"].mean().item()
            running_gt_loss += loss_dict["loss"].mean().item()
            ...
            if train_steps % args.log_every == 0:
                ...
                # dist.all_reduce(avg_reg_loss, op=dist.ReduceOp.SUM)
                dist.all_reduce(avg_gt_loss, op=dist.ReduceOp.SUM)
                
                avg_loss = avg_loss.item() / dist.get_world_size()
                # avg_reg_loss = avg_reg_loss.item() / dist.get_world_size()
                ...
                print_rank_0(f"(step={train_steps:07d}) Train Loss: {avg_gt_loss:.4f}, Total Loss: {avg_loss:.4f}, REG Loss: {avg_reg_loss:.4f}, Train Steps/Sec: {steps_per_sec:.2f}, LR: {opt.param_groups[0]['lr']:.6f}")
